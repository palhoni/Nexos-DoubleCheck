...
function validPlan(): TestPlan {
  return {
    ...
    casosRecomendados: [{ id: 'CTR-01', gapId: 'GAP-01', nome: 'Falha externa', categoria: 'Erro', escopo: 'Backend', precondicoes: [], passos: ['Executar'], resultadoEsperado: 'Erro tratado', automacao: 'Automatizável', prioridade: 'Alta' }],
    bloqueadores: [], checklist: { bloqueadores: [], ordemImplementacao: ['CTR-01'] }, frontendForaEscopo: [],
    totais: { requisitos: 6, cobertos: 6, gaps: 1, casosRecomendados: 1, bloqueadores: 0, frontend: 0 },
  };
}
...
describe('parseAndValidateTestPlan', () => {
  ...
  it('recupera somente os casos completos anteriores ao corte', () => {
    ...
    const result = parseAndValidateTestPlan(raw, 'US-1');
    expect(result.plan.casosRecomendados.map((item) => item.id)).toEqual(['CTR-01']);
  });

  it('rejeita totais divergentes mesmo quando o JSON é válido', () => {
    const plan = validPlan(); plan.totais.casosRecomendados = 14;
